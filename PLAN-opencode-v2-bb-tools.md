...
| Method | Purpose |
| --- | --- |
| `hello` | Protocol name, version, per-activation generation, and feature flags. `richFailures` comes from a live probe; this spike reports false |
| `attach` | Validate the catalog, replace any previous binding for that session, await `tool.reload()`, and return `bindingID`, capability, the accepting generation, and a numeric epoch |
| `status` | `{ capability }` → `{ bound, generation, epoch? }`. Unknown capability is unbound |
| `configure` | Replace the binding's current `disallowedTools` set |
| `pending` | Authoritative snapshot of calls (origin, no `turnId`) plus settled notices. `acknowledged` keys drop receipts and those notices. Polling stays authoritative; `control` events are a hint |
| `claim` | Atomically reserve one pending call for this capability |
| `result` | Submit one typed terminal result for a claim. An identical repeat is acked. A conflicting payload returns `conflict`. Receipts stay until `pending.acknowledged` lists the key, or the epoch ends |
...
- Plan agent plus `permissionMode: "full"` still advertising `edit` is a pre-existing limitation and out of scope.
...
| Permissions | Plan agent keeps bb tools; accept-edits/auto/full modes and canonical wholly-deny rules remain enforced; direct-origin guard rejects two same-owner inner Code Mode calls; no duplicate generic approval; auxiliary compaction/generation has no companion catalog. Plan agent plus `permissionMode: "full"` still advertising `edit` is pre-existing and out of scope |
