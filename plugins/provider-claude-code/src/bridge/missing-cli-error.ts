...
export class MissingClaudeCliError extends Error {
  constructor(cause: Error) {
    super(MISSING_CLAUDE_CLI_GUIDANCE, { cause });
    this.name = "MissingClaudeCliError";
  }
}
...
export function translateMissingClaudeCliCatalogError(error: unknown): unknown {
  if (error instanceof Error && isMissingClaudeCliMessage(error.message)) {
    return new MissingClaudeCliError(error);
  }
  return error;
}
