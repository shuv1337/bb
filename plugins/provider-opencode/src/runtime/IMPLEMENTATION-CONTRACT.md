...
- `resync` (either reason) → `session.context()`, then `translator.reconcileAfterResync` after the durable log's terminal tool and execution events are noted. A tool closes only when that log (or an execution `failed`/`interrupted`) says it ended, with that status and error text. Open tools are not marked completed because the stream restarted. A turn boundary is emitted only when the log shows the execution ended and no tool row is still open; an incomplete log never closes a turn as completed. The last context message id becomes the checkpoint. Message content is not copied into tool arguments. Events dropped by the outage or the overflow are not replayed. A turn still running in OpenCode stays open and continues on its next event. The whole reconcile is deferred while that turn has a pending or claimed companion call.
