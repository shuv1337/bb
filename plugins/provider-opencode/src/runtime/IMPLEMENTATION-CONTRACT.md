...
Plugin-local schemas live in `src/tool-bridge-contract.ts`. Golden fixtures are vendored at `src/fixtures/bb-tools-v1/` with `COMPANION_VERSION` pinning package `opencode-bb-tools` `0.1.0-dev` at commit `7f8eb66`. The conformance test checks every fixture and the requests the client sends.
