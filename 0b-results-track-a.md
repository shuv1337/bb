...
- `thread/stop` does not wait for `session.tool.failed` before returning. The companion `result` is awaited first, so the failure is already delivered; an SSE wait could miss an event that already fired and then block stop for seconds. With no open companion call, stop does no extra wait. The pre-existing interrupt settlement timeout (5s) still applies only when `intent` is `interrupt` and the session has interruptible work.
